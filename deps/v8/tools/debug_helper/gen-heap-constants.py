...
def iterate_objects(target_space, camel_space_name):
  global out
  result = [(offset, name) for (space, offset), (
      instance_type,
      name,
  ) in v8heapconst.KNOWN_MAPS.items() if space == target_space]
  ...
  for offset, name in result:
    out = f'{out}    case {str(offset)}: return "{name}' + '";\n'
...
def iterate_maps(target_space, camel_space_name):
  ...
  for (space, offset), (instance_type, name) in v8heapconst.KNOWN_MAPS.items():
    if space == target_space:
      out = (f'{out}    case {str(offset)}: return {str(instance_type)}' + ';\n')
...
if (hasattr(v8heapconst, 'HEAP_FIRST_PAGES')):  # Only exists in ptr-compr builds.
  ...
  out = out + '  }\n'
  expected_spaces = {'map_space', 'read_only_space', 'old_space'}
  for offset, space_name in v8heapconst.HEAP_FIRST_PAGES.items():
    if (space_name in expected_spaces):
      out = f'{out}  if (heap_addresses->{space_name}' + '_first_page == 0) {\n'
      out = (
          f'{out}    heap_addresses->{space_name}_first_page = i::DecompressTaggedPointer(any_uncompressed_ptr, {str(offset)}'
          + ');\n')
      out = out + '  }\n'
out = out + '}\n'
